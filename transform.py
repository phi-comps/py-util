...
x0_ = x1 + alpha
y0_ = y1
p1_ = p0_.subs(x0, x0_).subs(y0, y0_).expand().simplify()

x1_ = 1/x2
y1_ = y2/x2**2
# p2_ = x2**4 * p1_.subs(x1, x1_).subs(y1, y1_)
p2_ = p1_.subs(x1, x1_).subs(y1, y1_).expand().simplify().as_numer_denom()[0]

x2_ = x3
# y2_ = sqrt(p2_.coeff(x2**4))*y3
y2_ = sqrt(p2_.as_poly(x2).as_expr().coeff(x2**4))*y3
p3_tmp = p2_.subs(x2, x2_).subs(y2, y2_)
# p3_ = p3_tmp / p3_tmp.coeff(x3**4)
p3_ = (p3_tmp / p3_tmp.coeff(x3**4)).expand()
...
p4_ = x3**4 + f3*x3**3 + f2*x3**2 + f1*x3 + f0

g_ = x3**2 + g1*x3 + g0
h_ = h1*x3 + h0

eqns = nontriv(equate((g_**2 + h_).expand(), p4_, x3))
...
p5_ = 2*y4**2 + 2*g1*x4*y4 + 2*g0*x4**2 - x4**3 + h1*y4 + h0*x4
...
x4_ = x5/2
y4_ = y5/4
p6_ = p5_.subs(x4, x4_).subs(y4, y4_).expand().simplify()*8

p6__ = p6_.subs(g0, g0_).subs(g1, g1_).subs(h0, h0_).subs(h1, h1_).subs(f0, f0_).subs(f1, f1_).subs(f2, f2_).subs(f3, f3_).subs(x5, x).subs(y5, y)
...
def transform(c0_, c1_, c2_, c3_, c4_, alpha_):
    curve = -p6__.subs(c0, c0_).subs(c1, c1_).subs(c2, c2_).subs(c3, c3_).subs(c4, c4_).subs(alpha, alpha_)
